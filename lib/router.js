 ...
 *    - `action`: An optional function to call before instantiating
 *      `component`. The function may return a Promise if it depends on
 *      asynchronous behaviour to complete.
 */
...
/**
 * Returns a Promise for the top-level React Component configured for the
 * route, based on the passed-in `location`, a String href or Location object.
 */
Router.prototype.getComponent = function getComponent(location) {
  ...
  return when(route.config.action(route.params))
    .then(function () {
      return component({
        status: 200,
        error: null,
