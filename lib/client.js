...
Client.prototype.renderApplication = function renderApplication(location, perform) {
  ...
  return when(promise)
    .then(function () {
      ...
      React.render(route.body(props), document.body);
...
Client.prototype.start = function start() {
  ...
  self.window.onpopstate = function () {
    self.renderApplication(self.window.location, true);
  };
  self.window.document.addEventListener('DOMContentLoaded', function () {
    self.renderApplication(self.window.location, false);
  });
