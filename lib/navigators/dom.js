...
DomNavigator.prototype.navigate = function navigate(location, options) {
  ...
  if (!self._isInternalLocation(location)) {
    return when.reject(new Error('Cannot navigate to external location.'));
  }
...
/**
 * Returns `true` if the given String or Location is internal to the current
 * site, `false` otherwise.
 */
DomNavigator.prototype._isInternalLocation = function _isInternalLocation(location) {
  ...
  if (!this.context.router.isSameDomain(location, global.location)) {
    return false;
  }

  if (String(location).slice(0, 'javascript:'.length) === 'javascript:') {
    return false;
  }
