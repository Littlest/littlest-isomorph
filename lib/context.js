 ...
 * Context will be confined to the child, and lost whenever the child is
 * destroyed.
 *
 * If provided, `options` will override any options this Context would
 * otherwise have provided (or not) to the child.
 *
 * If `options[name]` is provided and a `name` attachment has been created,
 * those options will be passed on to the new child Context's instances of
 * that attachment.
 */
Context.prototype.getChild = function getChild(options) {
  ...
  context._cloneFrom(this, options);
...
Context.prototype.createAction = function createAction(name, fn) {
  function step(parent) {
    ...
    ctx.actions[name] = ctx.dispatcher.createAction(name, function action(scene) {
      scene.context = ctx;
...
/**
 * Attaches `value` to Context, making it accessible as if a singleton. More
 * importantly, these attachments carry on to all child Contexts. If a similar
 * `getChild(options)` method exists on `value`, that method will be called
 * within any of this Context's `getChild` calls, passed `options[name]`.
 */
Context.prototype.createAttachment = function createAttachment(name, value) {
  var shouldFork = (value && typeof value.getChild === 'function');

  function step(parent, options) {
    var ctx = this;

    if (parent && shouldFork) {
      ctx[name] = parent[name].getChild(options && options[name]);
      return;
    }
 ...
 */
Context.prototype._cloneFrom = function _cloneFrom(parent, options) {
  var self = this;

  self._cloneSteps.forEach(function (step) {
    step.call(self, parent, options);
  });
