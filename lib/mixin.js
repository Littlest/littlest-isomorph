...
var Mixin = function (React) {
  return {
    ...
    handle: function (path, handler) {
      ...
      // TODO(schoon) - Support multiple handlers for the same event.
    ...
    unhandle: function (path, handler) {
      ...
      // TODO(schoon) - Support multiple handlers for the same event.
      delete this._contextListeners[path];
    },
    ...
    map: function (path, key) {
      ...
      if (self._contextBindings[key]) {
        self.unhandle(eventPath, self._contextBindings[key]);
      }

      self.handle(eventPath, update);
      self._contextBindings[key] = update;
